				...
				if (i + 1 === data[j]['id']) {
					...
					$('#' + newId).on('click', function () {
						...
						$('#addAnswerToQuest').on('click', function () {
							...
							for (var i = 0; i < chetAnsw; i++) {
								if ($('.menu' + i).attr('value') == newAnswerVal) {
									...
									$.ajax({
										type: 'GET',
										url: "https://last-api.herokuapp.com/api/v1/answer/" + String(newAnswerVal),
										dataType: 'json',
										success: function (data) {
											let goto;
											goto = data['goto'];
						...
						$('#saveEdiBtn').on('click', function () {
							...
							$.ajax({
								...
								success: function () {
									fetch('https://last-api.herokuapp.com/api/v1/question/' + String(Number(newId.slice(7)) + 1) + '/edit', {
										method: 'PUT',
										headers: {
						...
						for (let i = 0; i < answList[Number(newId.slice(7))].length; i++) {
							...
							$.ajax({
								type: 'GET',
								url: "https://last-api.herokuapp.com/api/v1/answer/" + question,
								dataType: 'json',
								success: function (data) {
					...
					$('#' + newIdDelete).on('click', function () {
						...
						$('#closePopUpDelete1').on('click', function () {
							$.ajax({
								type: 'DELETE',
								url: 'https://last-api.herokuapp.com/api/v1/question/' + String(i + 1) + '/delete',
				...
				if (i + 1 === data[j]['id']) {
					...
					$('#' + newIdDeleteAnsw).on('click', function () {
						...
						$('#closePopUpDelete3').on('click', function () {
							$.ajax({
								type: 'DELETE',
								url: 'https://last-api.herokuapp.com/api/v1/answer/' + String(i) + '/delete',
								success: function (result) {
									alert(result['response']);
									window.location.href = "main.html";
								}
					...
					$('#' + newIdAnsw).on('click', function () {
						...
						$('#saveEditBtnAnsw').on('click', function () {
							...
							$.ajax({
								...
								data: { "username": "admin", "password": "SherBot" },
								success: function () {
									fetch('https://last-api.herokuapp.com/api/v1/answer/' + String(Number(newIdAnsw.slice(11)) + 1) + '/edit', {
										method: 'PUT',
										headers: {
											'Content-Type': 'application/json'
										},
